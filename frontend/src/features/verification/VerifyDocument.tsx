...
export default function VerifyDocument() {
  ...
  const submit = (e: React.FormEvent) => { e.preventDefault(); const normalized = code.trim().toUpperCase(); navigate(`/verify/${encodeURIComponent(normalized)}`); void verify(normalized); };
  const download = async () => {
    if (!result || activeTab === 'overview') return;
  ...
  const activeAttachment = !result ? null
  ...
  return <>
    <section className="relative isolate overflow-hidden bg-emerald-950 px-4 pb-24 pt-16 text-white sm:pt-24 print:hidden">
      ...
      <div className="mx-auto max-w-4xl text-center animate-rise"><div className="mx-auto mb-6 flex w-fit items-center gap-2 rounded-full border border-emerald-400/25 bg-emerald-400/10 px-4 py-2 text-xs font-bold text-emerald-200"><span className="h-2 w-2 animate-pulse rounded-full bg-emerald-400" />Official national verification service</div>
        ...
        <form onSubmit={submit} className="mx-auto mt-9 flex max-w-2xl flex-col gap-3 rounded-2xl bg-white p-2 shadow-2xl shadow-black/25 sm:flex-row"><label className="sr-only" htmlFor="code">Verification code</label><div className="flex flex-1 items-center gap-3 px-3"><svg viewBox="0 0 24 24" className="h-5 w-5 text-slate-400" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="11" cy="11" r="7"/><path d="m16 16 4 4"/></svg><input id="code" value={code} onChange={e=>setCode(e.target.value.replace(/[^a-zA-Z0-9-]/g,'').toUpperCase())} maxLength={16} placeholder="Enter code, e.g. KU-491029481" className="min-w-0 flex-1 bg-transparent py-3 font-mono text-base font-bold tracking-widest text-slate-900 outline-none placeholder:font-sans placeholder:font-normal placeholder:tracking-normal" /></div><button disabled={loading} className="rounded-xl bg-emerald-700 px-6 py-3.5 font-bold text-white transition hover:bg-emerald-800 disabled:opacity-60">{loading?'Checking…':'Verify record'}</button></form>
    ...
    <section className="print-shell relative z-10 mx-auto -mt-12 min-h-40 max-w-5xl px-4 pb-20 sm:px-6">
      ...
      {result && <article ref={resultRef} className="print-document scroll-mt-6 animate-rise overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl shadow-slate-900/10"><div className="print-document-body p-5 sm:p-9"><DocumentHeader data={result} />
