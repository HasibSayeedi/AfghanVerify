import type { CertificateData, Grade } from '../../../types';
...
export default function TranscriptTable({ data }: { data: CertificateData }) {
  ...
  return <div className="credential-transcript mx-auto w-full rounded-lg bg-gray-50 p-2 sm:p-3">
    <div className="credential-transcript-paper relative w-full overflow-hidden rounded-lg bg-white p-4 shadow-md sm:p-6">
    ...
    <header className="relative border-b-2 border-emerald-900 px-20 pb-6 text-center">
      <MinistryLogo className="absolute left-0 top-1/2 h-20 w-20 -translate-y-1/2 bg-transparent object-contain"/>
      <UniversityLogo code={data.university.code} name={data.university.nameEnglish} className="absolute right-0 top-1/2 h-20 w-20 -translate-y-1/2 object-contain"/>
      <p className="text-[10px] font-black uppercase tracking-[.3em] text-emerald-800">Islamic Emirate of Afghanistan</p>
      <h3 className="mt-2 font-serif text-2xl font-black text-emerald-950 sm:text-3xl">Official Academic Transcript</h3>
    ...
    {!data.transcript.length ? <div className="relative my-8 rounded-xl border border-dashed border-slate-300 p-8 text-center text-sm text-slate-500">No transcript courses are attached to this record.</div> :
      <div className="semester-grid relative mt-7 grid w-full grid-cols-1 gap-6 md:grid-cols-2">{Array.from(groups.entries()).sort(([a],[b])=>a-b).map(([semester,grades])=>{
        ...
        return <section key={semester} className="break-inside-avoid overflow-hidden rounded-xl border border-emerald-950/30 bg-white/80">
          ...
          <div className="w-full overflow-hidden"><table className="w-full table-fixed text-left text-sm"><thead><tr className="border-b border-emerald-950/20 bg-amber-50/70 text-[10px] uppercase tracking-[.08em] text-slate-600"><th className="w-12 px-2 py-3 text-center">No.</th><th className="px-3 py-3">Course title</th><th className="w-16 whitespace-nowrap px-1 py-3 text-center">Credits</th><th className="w-24 whitespace-nowrap py-3 pl-2 pr-4 text-center">Score / 100</th></tr></thead><tbody className="divide-y divide-slate-200">{grades.map((grade,index)=><tr key={`${semester}-${grade.subjectName}-${index}`}><td className="w-12 px-2 py-3 text-center text-slate-500">{index+1}</td><td className="break-words px-3 py-3 font-semibold leading-5 text-slate-900">{grade.subjectName}</td><td className="w-16 px-1 py-3 text-center font-mono tabular-nums">{grade.creditHours}</td><td className="w-24 py-3 pl-2 pr-4 text-center font-black tabular-nums text-emerald-800">{grade.score}</td></tr>)}</tbody><tfoot><tr className="border-t border-emerald-950/20 bg-emerald-50/60 text-xs font-black"><td colSpan={2} className="px-3 py-3 text-right uppercase tracking-wide">Semester summary</td><td className="w-16 px-1 py-3 text-center tabular-nums">{semesterCredits}</td><td className="w-24 py-3 pl-2 pr-4 text-center tabular-nums">{semesterAverage.toFixed(1)}%</td></tr></tfoot></table></div>
        </section>;
      })}</div>}
